...
const ManageApprovedCiphers = () => {
    ...
    useEffect(() => { fetchCiphers(); }, [fetchCiphers]);
    ...
    const openDeleteModal = useCallback((id) => setDeleteModal({ open: true, id }), []);
    const closeDeleteModal = useCallback(() => setDeleteModal({ open: false, id: null }), []);
    ...
    const renderActions = (cipher) => (
        <div className="action-group">
            <button className="btn-icon" title="Редактирай" onClick={() => openEditModal(cipher)}>
        ...
        <div className="table-toolbar">
            <div className="toolbar-left">
                <div className="filter-tabs">
                    <button className={`filter-tab${challengeTypeFilter === 0 ? ' active' : ''}`} onClick={() => setChallengeTypeFilter(0)}>Стандартен</button>
                    <button className={`filter-tab${challengeTypeFilter === 1 ? ' active' : ''}`} onClick={() => setChallengeTypeFilter(1)}>Експериментален</button>
                </div>
                <div className="search-input-wrap">
                    <svg className="search-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5">
                        <circle cx="7" cy="7" r="5"/><path d="M11 11l3 3"/>
                    </svg>
                    <input type="text" className="form-input" placeholder="Търсене по заглавие, вид..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                </div>
            </div>
            <div className="toolbar-right">
                <select className="form-select" style={{ width: '140px' }} value={tagsFilter} onChange={(e) => setTagsFilter(parseInt(e.target.value))}>
                    <option value="0">Никакъв</option>
                    {AVAILABLE_TAGS.map((tag) => (
                        <option key={tag.value} value={tag.value}>{tag.label}</option>
                    ))}
                </select>
                <select className="form-select" style={{ width: '140px' }} value={orderTerm} onChange={(e) => setOrderTerm(parseInt(e.target.value))}>
    ...
    const emptyOrError = error ? (
        ...
        <div className="data-table-wrapper">
            <div className="empty-state">
                ...
                <div className="empty-state-text">{searchTerm ? 'Няма резултати, съответстващи на търсенето' : 'Все още няма одобрени шифри'}</div>
            </div>
        </div>
    ) : null;

    return (
        <div className="admin-shell admin-shell--manage-ciphers">
                ...
                <div className="admin-content">
                    ...
                    {emptyOrError || (
                        <>
                            ...
                            <div className="data-table-wrapper">
                                <table className="data-table">
                                    ...
                                    <tbody>
                                        {ciphers.map((cipher) => (
                                            <tr key={cipher.id}>
                                                <td className="mono" style={{ color: 'var(--text-dim)', fontSize: '11px' }}>#{cipher.id}</td>
                                                <td>
                                                    <div className="cipher-title-cell">{cipher.title || `Шифър #${cipher.id}`}</div>
                                                    {cipher.isImage && (
                                                ...
                                                <td>
                                                    {cipher.cipherType
                                                        ? <span className="mono" style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>{cipher.cipherType}</span>
                                                        : <span style={{ color: 'var(--text-dim)', fontSize: '11px' }}>—</span>}
                                                </td>
                                                <td>
                                                    {cipher.percentageOfConfidence
                                                        ? <span className="mono" style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>{cipher.percentageOfConfidence}%</span>
                                                        : <span style={{ color: 'var(--text-dim)', fontSize: '11px' }}>—</span>}
                                                </td>
                            ...
                            <div className="ciphers-card-list">
                                {ciphers.map((cipher) => (
                                    <div key={cipher.id} className="cipher-card">
                                        <div className="cipher-card__row">
                                            <span className="cipher-card__label">Заглавие</span>
                                            <span className="cipher-card__value">{cipher.title || `Шифър #${cipher.id}`}</span>
                                        </div>
                                        {cipher.cipherType && (
                                            <div className="cipher-card__row">
                                                <span className="cipher-card__label">Вид</span>
                                                <span className="cipher-card__value cipher-card__value--dim">{cipher.cipherType}</span>
                                            </div>
                                        )}
                                        <div className="cipher-card__row">
                                            <span className="cipher-card__label">Автор</span>
                                            <span className="cipher-card__value cipher-card__value--dim">{cipher.submittedBy || '—'}</span>
                                        </div>
                                        <div className="cipher-card__actions">
                                            {renderActions(cipher)}
                                        </div>
                                    </div>
                                ))}
                            </div>
                                ...
                                <div className="form-group">
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '8px' }}>
                                        <label className="edit-permission-toggle">
                                            <input type="checkbox" checked={editAllowTypeHint} onChange={(e) => setEditAllowTypeHint(e.target.checked)} />
                                            <span>Позволи подсказки за вида</span>
                                        </label>
                                        <label className="edit-permission-toggle">
                                            <input type="checkbox" checked={editAllowHint} onChange={(e) => setEditAllowHint(e.target.checked)} />
                                            <span>Позволи подсказки</span>
                                        </label>
                                        <label className="edit-permission-toggle">
                                            <input type="checkbox" checked={editAllowSolution} onChange={(e) => setEditAllowSolution(e.target.checked)} />
