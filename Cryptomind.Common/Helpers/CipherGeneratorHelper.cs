...
namespace Cryptomind.Common.Helpers
{
	public static class CipherGeneratorHelper
	{
		...
		public static string MorseCode(string plaintext)
		{
			return string.Join(" ", plaintext.ToUpper()
				.Select(c => c == ' ' ? "/" : MorseDict.TryGetValue(c, out var code) ? code : "")
				.Where(s => s != ""));
		}

		public static string BinaryEncode(string plaintext)
			=> string.Join(" ", plaintext.Select(c => Convert.ToString(c, 2).PadLeft(8, '0')));

		public static string HexEncode(string plaintext)
			=> Convert.ToHexString(Encoding.UTF8.GetBytes(plaintext)).ToLower();
